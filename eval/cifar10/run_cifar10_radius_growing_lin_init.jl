...
function run_cifar10_radius_growing_lin_init_experiment(; patience=2, properties=1:100, n_unfixed=[27], radii=5:50)
    ...
    logfile = logfile_prefix * "logs_lin_init_radius_" * date_string * ".csv"

    open(logfile, "w") do f
        println(f, "network,property,n_unfixed,radius,result,time,steps,hist_file")
    end
        ...
        for prop in properties
            ...
            for n_un in n_unfixed
                for r in radii
                    ...
                    open(logfile, "a") do f
                        println(f, string(model_name, ", ", prop, ", ", n_un, ",", r, ", ", verified, ", ", t, ", ", length(res.t_hist), ",", hist_file_name))
                    end
