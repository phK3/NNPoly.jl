function [res, x, bounds_hist, t_hist] = verify_bounds(nn, X0, spec, varargin)
...
x = [];  % we only do incomplete verification here

% 1. parse input
if nargin < 3
    throw(CORAerror("CORA:notEnoughInputArgs", 3))
end
[nn,X0,spec,splits,numRefineSteps,verbose,doPlot,plotDimsIn,plotDimsOut,timeout] = ...
...
while ~isempty(unknownSets) && splits >= 0
    ...
    for i = 1:length(unknownSets)
        ...
        % we only want incomplete verification here, so ignore
        % falsification
        ...
        % b) try to verify set --------------------------------------------
        [res, Y, bounds_hist, t_hist] = aux_verify(nn, X, spec, numRefineSteps, verbose, timeout);
...
function [nn,X0,spec,splits,numRefineSteps,verbose,doPlot,plotDimsIn,plotDimsOut,timeout] = ...
    aux_parseInput(nn,X0,spec,varargin)
    ...
    [varargin,timeout] = readNameValuePair(varargin,'Timeout',{'isnumeric','isscalar'},120);
...
function [res, Yout, bounds_hist, t_hist] = aux_verify(nn, X, spec, numRefineSteps, verbose, timeout)
...
for i = 1:numRefineSteps
    ...
    % for polytope output spec, matrix is used as the last layer of the
    % polytope -> just check interval bounds <= 0
    ...
    % res = contains(spec,Y);
    % (contains(set, S))
    % which checks all of the half spaces in the polytope
    % res = check(spec, Y);
    % but we want the bounds
    violations = Y.sup;

    viosum = sum(max(0, violations));
    bounds_hist(i) = viosum;
    t_hist(i) = toc();
    if verbose && t_hist(i) >= print_step
        print_step = print_step + 5;
        gl = strjoin(string(Y.inf)); %sprintf('%d', Y.inf);
        gu = strjoin(string(Y.sup)); %sprintf('%d', Y.sup);
        fprintf('----------- %d sec --------\n', t_hist(i))
        fprintf('lower bounds: %s\n', gl)
        fprintf('violations:   %s\n', gu)
        fprintf('sum(violations): %d\n', viosum)
    end
