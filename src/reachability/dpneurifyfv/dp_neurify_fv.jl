...
@with_kw struct DPNFV <: NV.Solver
...
function NV.forward_linear(solver::DPNFV, L::NV.LayerNegPosIdx, input::SymbolicIntervalFVHeur)
    Low, Up = NV.interval_map(L.W_neg, L.W_pos, input.Low, input.Up)
    Low[:, end] .+= L.bias
    Up[:, end] .+= L.bias
...
function NV.forward_act(solver::DPNFV, L::NV.LayerNegPosIdx{NV.ReLU}, input::SymbolicIntervalFVHeur)
    n_node = NV.n_nodes(L)
    ...
    if solver.method == :OuterBounds
        λ_l = NV.relaxed_relu_gradient_lower.(low_low, up_up)
        λ_u = NV.relaxed_relu_gradient.(low_low, up_up)
        β_u = -low_low
    elseif solver.method == :NeurifyRelax
        λ_l = NV.relaxed_relu_gradient.(low_low, low_up)
        λ_u = NV.relaxed_relu_gradient.(up_low, up_up)
        β_u = -up_low
    elseif solver.method == :DeepPolyRelax
        λ_l = NV.relaxed_relu_gradient_lower.(low_low, low_up)
        λ_u = NV.relaxed_relu_gradient.(up_low, up_up)
        β_u = -up_low
    end
...
function NV.forward_act(solver::DPNFV, L::NV.LayerNegPosIdx{NV.Id}, input::SymbolicIntervalFVHeur)
    n_node = NV.n_nodes(L)
    n_sym = get_n_sym(input)
    n_in = get_n_in(input)
    current_n_vars = get_n_vars(input)
