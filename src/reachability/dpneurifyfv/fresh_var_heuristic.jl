...
function backward_linear(solver::FreshVarHeuristic, L::NV.Layer, input::HeuristicScore)
    W⁺ = max.(0., L.weights')
...
function backward_act(solver::FreshVarHeuristic, L::NV.Layer{NV.ReLU}, input, lbs, ubs)
...
function backward_act(solver::FreshVarHeuristic, L::NV.Layer{NV.Id}, input, lbs, ubs)
    return input
end
