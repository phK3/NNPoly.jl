...
"""
function init_symbolic_interval_fvheur(net::NV.NetworkNegPosIdx, input_set::AbstractPolytope{N}; max_vars=10) where N <: Number
    n = dim(input_set)
    Low = [I zeros(N, n)]
    Up = [I zeros(N, n)]

    lbs = [fill(typemin(N), NV.n_nodes(l)) for l in net.layers]
    ubs = [fill(typemax(N), NV.n_nodes(l)) for l in net.layers]
